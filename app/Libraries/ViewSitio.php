...
class ViewSitio
{
    ...
    public function getMenuConvocatorias($menuBloque)
    {
        ...
        if ($menuBloque['menu'] == 2) { // para traer solo un resultado para convocatoria
            ...
            $selesccionCategorias = $categoria->select($array)->where('edi_cat', 4)->orderby('ord_cat')->findAll();
            // print_r($selesccionCategorias);
            // return '';
            return view('components/cabezote/menuprincipal/menu3', ['menu' => $menuprincupal[0],  'selesccionCategorias' =>  $selesccionCategorias, 'submenus' => $submenudata[0]]);
            // return '';
        }
    }
    ...
    public function menuagendaeventos()
    {
        ...
        $menu = new Menuprincipal();
        $array = ['nom_mep'];
        $data = $menu->select($array)->where('cod_mep', $edicionActual)->findAll();
    ...
    // public function representantes($crds){
    //     return view('components/cabezote/menuprincipal/menu5', ['menu' => $menuprincupal[0], 'submenus' => $submenudata[0], 'menuActivo' => $menuActivo]);
    // }
}