...
class CategoriasModel extends Model
{
    ...
    public function listatardatos($id = 0)
    {
        ...
        if ($id == 0) {
            ...
            foreach ($data['data'] as $fila) {
                $filapadre = $this->where('cod_pro', $fila['pad_pro'])->findAll();
                foreach ($filapadre as $padre) {
                    $fila['proceso'] = $padre['nom_pro'];
                }
                $datos[]  = $fila;
                $data['selprocesos'] = $this->where('pad_pro', 0)->findAll();
            }
            $data['data'] = $datos;
            // print_r($data);
        } else {
    ...
    public function guardar($request)
    {
        $data = [
            ...
            'pad_pro' => $request->getVar('codproceso')
        ];
    ...
    {
        $id = $request->getVar('codigoEliminar');
        $id = $this->delete($id);
        return $id;
    }
}