...
function getTitleFancy() {
	...
	if (title !== undefined) {
		var htmlTitle = '<div class="cFDetails cText">' + title + "</div>";
		return htmlTitle;
	} else {
		return false;
	}
...
(function ($) {
	$.createAlert = function (opt_user) {
		...
		if (options.title !== false) {
			conTitulo =
				'<div class="gATitle"><h3 class="titilealerta" >' +
				options.title +
				"</h3></div>";
		}
		if (options.cancelButton === true || options.acceptButton === true) {
			...
			if (options.cancelButton === true) {
				conButtons =
					conButtons +
					'<button id="btnCancel" class="gButton btnING-ven" type="button"><span>' +
					options.labelCancel +
					"</span></button>";
			}
			if (options.acceptButton === true) {
				conButtons =
					conButtons +
					'<button id="btnAccept" class="gButton btnING-ven" type="button"><span>' +
					options.labelAccept +
					"</span></button>";
			}
	...
	if (rangeCalendar.length) {
		rangeCalendar.each(function () {
			...
			thisCalendar.daterangepicker({
				presetRanges: false,
				initialText: "B̼squeda por fecha",
				icon: "",
...
function loadICalendar() {
	...
	if (iCalendar.length) {
		iCalendar.each(function () {
			var thisCalendar = $(this),
				thisYearRange = thisCalendar.data("range"),
				yearRange = "c-10:c+10";
			if (thisYearRange !== undefined) {
				yearRange = thisYearRange;
			}
			...
			thisCalendar.on("focus", function () {
				$(this).blur();
			});
		});
	}
	...
	if (sliderPrefooter.length) {
		sliderPrefooter.slick({
			...
			responsive: [
				{
					breakpoint: 768,
					settings: { adaptiveHeight: true, autoplay: true },
				},
			],
		});
	}
